...
# Key changes applied:
# 1. Removed multiprocessing → switched to sequential transcription
#    → Models load ONLY ONCE, no repeated loading, no memory explosion
# 2. Vosk fallback is fully commented out (easy to re-enable by uncommenting)
# 3. Kept "small" Whisper model + CPU mode for maximum stability on Windows
# 4. Minor cleanups and comments for clarity
...
def vad_and_diarize(audio_file: str) -> List[Dict]:
    ...
    embeddings = []
    valid_indices = []  # Keep track of which segments we kept for clustering
    ...
    if not embeddings:
        # Fallback: treat whole file as one speaker
    ...
    for i, seg in enumerate(segments):
        if i in valid_indices:
            speaker = f"Speaker_{labels[label_idx] + 1}"
            label_idx += 1
        else:
            speaker = "Speaker_1"  # Short segments → default speaker
        diarized_segments.append({"start": seg['start'], "end": seg['end'], "speaker": speaker})

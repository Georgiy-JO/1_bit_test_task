# Consolidated and fixed transcription pipeline script: transcriber.py
# Why a new approach?
...
compute_type = "float16" if device == "cuda" else "int8"  # Optimize for device
...
# Faster-Whisper (free, optimized Whisper variant)
print("[INIT] Loading Faster-Whisper model...")
whisper_model = WhisperModel("medium.en", device=device, compute_type=compute_type)  # .en for English, medium for balance

# Vosk (free, lightweight ASR)
print("[INIT] Loading Vosk model...")  # Download from https://alphacephei.com/vosk/models
vosk_model_path = "vosk-model-en-us-0.22"  # Assume downloaded and placed in cwd or path
if not os.path.exists(vosk_model_path):
    raise RuntimeError("Download Vosk model: https://alphacephei.com/vosk/models and set path.")
...
polish_pipe = pipeline("text2text-generation", model="google/flan-t5-small", device=0 if device == "cuda" else -1)

def cleanup_audio(input_file: str, output_file: str = "cleaned_audio.wav") -> str:
    """
    Cleanup: Convert to mono 16kHz WAV, apply RNNoise if needed.
    Why? FFMPEG is fast/system-level; RNNoise reduces noise locally (pro tools like Adobe Audition use similar).
    Tricky: Use subprocess for FFMPEG (no Python dep needed beyond install).
    """
    cmd = [
        "ffmpeg",
        "-i", input_file,
        "-af", "arnndn=m=rnnoise_model.pth",  # Noise suppression (download model if needed: https://github.com/GregorR/rnnoise-models)
        "-ac", "1",  # Mono
        "-ar", "16000",  # 16kHz for ASR models
        "-y", output_file
    ]
...
def transcribe_segment(args):
    ...
    # Whisper
    whisper_segments, _ = whisper_model.transcribe(wav_np, beam_size=5)
...
def polish_text(text: str) -> str:
    ...
    if not text:
        return ""
    prompt = f"Fix grammar and punctuation: {text}"
...
def transcribe_audio(input_audio: str, output_json: str = "transcript.json", use_polish: bool = True) -> None:
    """
    Main pipeline.
    """
...
if __name__ == "__main__":
    transcribe_audio("your_audio.mp3")  # Replace with input